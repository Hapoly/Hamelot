...
class Keys extends Controller{
  ...
  public function show(Key $key){
    return view('panel.keys.show', ['key' => $key]);
  }
  public function create(){
    return view('panel.keys.create',
      ['templates' => Template::where('status', Template::S_ACTIVE)->get()]);
  }
  public function store(KeyRequest $request){
    $key = Key::create($request->all());
    return redirect()->route('panel.keys.show', ['key' => $key]);
  }
  public function edit(Key $key){
    return view('panel.keys.edit', ['key' => $key, 'templates' => Template::where('status', Template::S_ACTIVE)->get()]);
  }
  ...
  public function destroy(Key $key){
    $key->delete();
    if(URL::route('panel.keys.show', ['key' => $key]) == URL::previous())
      return redirect()->route('panel.keys.index');
    else
