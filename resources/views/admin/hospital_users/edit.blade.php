    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.hospital_users.update', ['hospital_user' => $hospital_user]) }}" enctype="multipart/form-data">
                        {{ method_field('PUT') }}
                        @csrf
                          ...
                          <div class="col-md-6">
                              <select class="form-control" name="user_id" id="user_id">
                                @foreach($users as $user)
                                  <option value="{{$user->id}}" {{old('user_id', $hospital_user->user_id) == $user->id? 'selected': ''}} > {{$user->first_name}} {{$user->last_name}} </option>
                                @endforeach
                              </select>
                          ...
                          <div class="col-md-6">
                              <select class="form-control" name="hospital_id" id="hospital_id">
                                @foreach($hospitals as $hospital)
                                  <option value="{{$hospital->id}}" {{old('hospital_id', $hospital_user->hospital_id) == $hospital->id? 'selected': ''}} > {{$hospital->title}} </option>
                                @endforeach
                              </select>
