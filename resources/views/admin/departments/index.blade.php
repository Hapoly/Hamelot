...
@section('content')
  <div class="container">
    ...
    <div class="row justify-content-center">
      <div class="col-md-8">
        <form action="{{route('departments.index',['sort' => $sort])}}" method="get">
    ...
    @if(sizeof($departments))
      <table class="table table-striped">
        <thead>
          <tr>
            <th ><a href="{{route('departments.index',['search' => $search,'sort' => 'id'    ,'page' => $departments->currentPage()])}}">{{__('departments.row')}}</a></th>
            <th ><a href="{{route('departments.index',['search' => $search,'sort' => 'title'    ,'page' => $departments->currentPage()])}}">{{__('departments.title')}}</a></th>
            <th ><a href="{{route('departments.index',['search' => $search,'sort' => 'hospital_id'    ,'page' => $departments->currentPage()])}}">{{__('departments.hospital_id')}}</a></th>
            <th ><a href="{{route('departments.index',['search' => $search,'sort' => 'status'    ,'page' => $departments->currentPage()])}}">{{__('departments.status')}}</a></th>
        ...
        <tbody>
          @foreach($departments as $department)
            ...
            <td><a href="{{route('hospitals.show', ['hospital' => $department->hospital])}}">{{$department->hospital->title}}</a></td>
