@extends('layouts.app')
@section('title', $unit->complete_title)
@section('content')
<div class="container">
    <div class="row">
        <div class="col-md-12">
            <div class="show-card">
                <div class="row">
                    <div class="col-md-7">
                        <table class="table table-striped">
                            <tbody>
                                <tr>
                                    <td>نام پزشک</td>
                                    <td>{{$unit->members[0]->full_name}}</td>
                                </tr>
                                <tr>
                                    <td>تخصص ها</td>
                                    <td>{{$unit->members[0]->fields_str}}</td>
                                </tr>
                                <tr>
                                    <td>آدرس مطب</td>
                                    <td>{{$unit->address}}</td>
                                </tr>
                                <tr>
                                    <td>شماره همراه مطب</td>
                                    <td>{{$unit->phone_str}}</td>
                                </tr>
                                <tr>
                                    <td>شماره ثابت مطب</td>
                                    <td>{{$unit->phone_str}}</td>
                                </tr>
                                <tr>
                                    <td>استان</td>
                                    <td>{{$unit->city->province->title}}</td>
                                </tr>
                                <tr>
                                    <td>شهر</td>
                                    <td>{{$unit->city->title}}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    <div class="col-md-5" style="text-align: center; padding: 5px">
                        <img style="width: 50%" src="{{$unit->members[0]->doctor->profile_url}}" />
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-12">
                        <table class="table table-striped">
                            <thead>
                                <th>روز هفته</th>
                                <th colspan="5">ساعت کاری</th>
                            </thead>
                            <tbody>
                                @for($i=1; $i<=7; $i++)
                                    <tr>
                                        <td>{{__('general.day_of_week.' . $i)}} - {{$unit->activity_times[$i]['date']}}</td>
                                        <td>
                                            @php
                                                $day = $unit->activity_times[$i]['day'];
                                            @endphp
                                            <div class="row">
                                                @foreach($unit->activity_times[$i]['times'] as $time)
                                                    <a class="primary-btn" href="{{route('panel.demands.create.visit', ['activity_time' => $time, 'day' => $day])}}">
                                                        <span>
                                                        {{$time->day_less_time_str}} ({{$time->unit_user->user->full_name}})
                                                        </span>
                                                    </a>
                                                @endforeach
                                            </div>
                                        </td>
                                    </tr>
                                @endfor
                            </tbody>
                        </table>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-6" style="text-align: right">
                        @if($offset <= time())
                            <a class="default-btn">هفته قبل</a>
                        @else
                            <a href="{{route("show.unit", ['slug' => $unit->slug, 'time' => ($offset - (7*24*3600))])}}" class="default-btn">هفته قبل</a>
                        @endif
                    </div>
                    <div class="col-md-6">
                        <a href="{{route("show.unit", ['slug' => $unit->slug, 'time' => ($offset + (7*24*3600))])}}" class="default-btn">هفته بعد</a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

@endsection