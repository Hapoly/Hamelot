    ...
    <div class="col-md-8">
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div class="row">
            <a href="{{route('manager.users.edit', ['user' => $user])}}" class="btn btn-primary" role="button">{{__('users.edit')}}</a>
            <form action="{{route('manager.users.destroy', ['user' => $user])}}" method="post">
              {{ method_field('DELETE') }}
              {{ csrf_field() }}
                ...
                <tbody>
                  @foreach($user->hospitals as $hospital_user)
                    <tr>
                      ...
                      <td><a href="{{route('manager.hospitals.show', ['hospital_user' => $hospital_user])}}">{{$hospital_user->hospital->title}}</a></td>
                      <td>
                ...
                <tbody>
                  @foreach($user->departments as $department_user)
                    <tr>
                      ...
                      <td><a href="{{route('manager.departments.show', ['department_user' => $department_user])}}">{{$department_user->department->title}}</a></td>
                      <td>
