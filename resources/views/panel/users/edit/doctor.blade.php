...
@form_edit(['post' => true, 'action' => route('panel.users.update.doctor', ['user' => $user]), 'title' => __('users.edit.doctor')])
        ...
        <div class="form-group row create-form">
            ...
            @endphp
            @input_select(['name' => 'degree_id', 'value' => old('degree_id', $user->doctor->degree_id), 'label' => __('users.degree'), 'required' => true, 'rows' => $degree_rows, 'col' => 6])
            ...
            @endphp
            @input_select(['name' => 'field_id', 'value' => old('field_id', $user->doctor->field_id), 'label' => __('users.field'), 'required' => true, 'rows' => $field_rows, 'col' => 6])
        </div>
        @php
            $gender_rows = [
