...
<div class="container">
    @form_edit(['post' => true, 'action' => route('panel.users.update.doctor', ['user' => $user]), 'title' => __('users.edit.doctor')])
        ...
        @endphp
        @input_select(['name' => 'public', 'value' => old('public', $user->doctor->public, 0), 'label' => __('users.public'), 'required' => true, 'rows' => $public_rows, 'row' => true])
        @if(Auth::user()->isAdmin())        
            @php
                $status_rows = [
            ...
            @endphp
            @input_select(['name' => 'status', 'value' => old('status', $user->doctor->status, 0), 'label' => __('users.status'), 'required' => true, 'rows' => $status_rows, 'row' => true])
        @endif
