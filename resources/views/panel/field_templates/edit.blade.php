...
@section('content')
<div class="container">
  @form_edit(['action' => route('panel.field_templates.update', ['field_template' => $field_template]), 'title' => 'ویرایش ' . $field_template->title, 'row' => true])
    @input_text(['name' => 'title', 'value' => old('title', $field_template->title), 'label' => 'عنوان', 'required' => true, 'row' => true])
    @input_text(['name' => 'unit', 'value' => old('unit', $field_template->unit), 'label' => 'واحد', 'required' => true, 'row' => true])
    @input_text_area(['name' => 'description', 'value' => old('description', $field_template->description), 'label' => 'توضیحات', 'required' => true, 'row' => true, 'lines' => 5])
    @php
      $type_rows = [];
    ...
    @endphp
    @input_select(['name' => 'type', 'value' => old('type', $field_template->type), 'label' => 'نوع', 'required' => true, 'rows' => $type_rows, 'row' => true])
    @input_select(['name' => 'status', 'value' => old('status', $field_template->status), 'label' => 'وضعیت', 'required' => true, 'rows' => $status_rows, 'row' => true])
    <div id="ranges" class="form-row">
      @for($i=0; $i<sizeof(old('values', $field_template->ranges)); $i++)
          ...
          <div class='panel panel-default create-card' style='margin-top:30px; background: #EEE; border-color: #EEE;' >
            ...
            <div class='form-group row create-form'>
              <div class='col-md-12'>
                <div class='form-group test-in create-form' >
                  <div class='col-md-10'>
                    <input id='value' type='text' class='form-control' name='values[]' value="{{old('values.' . $i, $field_template->ranges[$i]->value)}}" style='width:90%;'>
                ...
                <div class='form-group test-in'>
                  <div class='col-md-10' style='padding-right: 25px;'>
                    <select class='form-control' name='modes[]' style='width:88%; text-align:center'>
                      <option value='1' {{old('modes.' . $i, $field_template->ranges[$i]->mode) == 1? 'selected': ''}} >حد بالا</option>
                      <option value='2' {{old('modes.' . $i, $field_template->ranges[$i]->mode) == 2? 'selected': ''}} >حد پایین</option>
                    </select>
            ...
            <div class='form-group row create-form'>
              <div class='col-md-12'>
                <div class='form-group test-in create-form' >
                  <div class='col-md-10'>
                    <input id='value' type='text' class='form-control' name='descriptions[]' value="{{old('descriptions.' . $i, $field_template->ranges[$i]->description)}}" style='width:90%;'>
                    @if($errors->has('values.' . $i))
                      <span class="invalid-feedback">
