    ...
    <div class="col-md-8">
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div class="row">
            <a href="{{route('panel.hospital_users.edit', ['hospital_user' => $hospital_user])}}" class="btn btn-primary" role="button">{{__('hospital_users.edit')}}</a>
            <form action="{{route('panel.hospital_users.destroy', ['hospital_user' => $hospital_user])}}" method="post">
              {{ method_field('DELETE') }}
              {{ csrf_field() }}
