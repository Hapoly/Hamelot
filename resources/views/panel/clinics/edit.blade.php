...
<div class="container">
  @form_edit(['action' => route('panel.policlinics.update', ['policlinic' => $policlinic]), 'title' => __('policlinics.edit')])
    ...
    @input_text(['name' => 'phone', 'value' => old('phone', $policlinic->phone == 'NuLL'? '': $policlinic->phone), 'label' => __('policlinics.phone'), 'required' => false])
    @input_text(['name' => 'mobile', 'value' => old('mobile', $policlinic->mobile == 'NuLL'? '': $policlinic->mobile), 'label' => __('policlinics.mobile'), 'required' => false])
    @tagline
