    ...
    <div class="col-md-8">
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div class="row">
            <a href="{{route('panel.templates.edit', ['template' => $template])}}" class="btn btn-primary" role="button">{{__('templates.edit')}}</a>
            <form action="{{route('panel.templates.destroy', ['template' => $template])}}" method="post">
              {{ method_field('DELETE') }}
              {{ csrf_field() }}
