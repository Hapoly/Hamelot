...
<div class="container">
    @form_edit(['post' => true, 'action' => route('panel.profile.patient'), 'title' => __('users.profile_edit')])
        ...
        @input_date(['name' => 'birth_', 'year' => old('birth_year', Auth::user()->birth_year), 'month' => old('birth_month', Auth::user()->birth_month), 'day' => old('birth_day', Auth::user()->birth_day), 'label' => __('users.birth_date'), 'row' => true])
        <div class="form-group row create-form">
            @php
                $gender_rows = [
            ...
            @endphp
            @input_select(['name' => 'gender', 'value' => old('gender', Auth::user()->patient->gender), 'label' => __('users.gender'), 'required' => true, 'rows' => $gender_rows, 'col' => 6])
            @php
                $status_rows = [
            ...
            @endphp
            @input_select(['name' => 'status', 'value' => old('status', Auth::user()->patient->status), 'label' => __('users.status'), 'required' => true, 'rows' => $status_rows, 'col' => 6])
        </div>
