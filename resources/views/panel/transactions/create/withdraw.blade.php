...
<div class="container">
  @form_create(['action' => route('panel.transactions.store.withdraw'), 'title' => __('transactions.create_withdraw')])
    ...
    @endphp
    @input_select(['name' => 'bank_account_id', 'value' => old('bank_account_id', ''), 'label' => __('transactions.bank_account_id'), 'required' => true, 'rows' => $bank_accounts_rows])
    ...
    @if(Auth::user()->isAdmin())
      ...
      @endphp
      @input_select(['name' => 'status', 'value' => old('status', ''), 'label' => __('transactions.status'), 'required' => true, 'rows' => $status_rows])  
    @endif
