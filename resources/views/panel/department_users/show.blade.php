    ...
    <div class="col-md-8">
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div class="row">
            <a href="{{route('panel.department_users.edit', ['department_user' => $department_user])}}" class="btn btn-primary" role="button">{{__('department_users.edit')}}</a>
            <form action="{{route('panel.department_users.destroy', ['department_user' => $department_user])}}" method="post">
              {{ method_field('DELETE') }}
              {{ csrf_field() }}
